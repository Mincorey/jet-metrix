...
import { FuelDispensingTZARecord } from '../data/Fuel_Dispensing_TZA';

interface FuelDispensingTZAReportProps {
...
export default function FuelDispensingTZAReport({ currentWorkday, onBack }: FuelDispensingTZAReportProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (blob && navigator.share) {
        const file = new File([blob], `Отчет_по_выдаче_в_ТЗА.png`, { type: 'image/png' });
        await navigator.share({
          files: [file],
          title: 'Отчет по выдаче в ТЗА',
          text: `Отчет по выдаче в ТЗА`
        });
      } else {
  ...
  const handleExportExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet('Выдача в ТЗА');
    ...
    worksheet.columns = [
      ...
      { header: 'Резервуар', key: 'tank', width: 20 },
    ...
    reportData.forEach((row) => {
      ...
      newRow.eachCell((cell, colNumber) => {
        ...
        let align = 'center'; // Дата
        if (colNumber >= 2 && colNumber <= 4) align = 'left'; // Сотрудник, ТЗА, Резервуар
        if (colNumber >= 5) align = 'right'; // Плотность, Объем, Масса
        ...
        if (typeof cell.value === 'number') {
          ...
          else if (colNumber >= 6) cell.numFmt = '#,##0.00'; // Объем и масса
        }
      });
    });
    ...
    saveAs(blob, `Отчет_по_выдаче_в_ТЗА_${new Date().toLocaleDateString('ru-RU')}.xlsx`);
  };
          ...
          <div className="min-h-screen px-4 flex flex-col items-center py-8">
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-md shadow-xl text-left my-auto">
              ...
              <div ref={resultRef} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 mb-6">
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 text-center mb-4">Отчет по выдаче в ТЗА</h3>
                    ...
                    reportData.map((row, idx) => (
                      <div key={idx} className="border-b border-slate-200 dark:border-slate-700 pb-4 last:border-0 last:pb-0">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-xs font-medium bg-amber-50 text-amber-700 px-2 py-1 rounded-md">ТЗА {row.TZA}</span>
                        </div>
                        <div className="space-y-1.5 pl-2 border-l-2 border-amber-100">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-slate-500 dark:text-slate-400 text-xs">Резервуар:</span>
                            <span className="font-medium text-slate-700 dark:text-slate-300 text-xs">{row.Tank_Name}</span>
                          </div>
                          <div className="flex justify-between items-center">
